...
/*   Updated: 2024/09/25 11:34:28 by ulmagner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	display_map(t_solong *solong, t_window *window, t_image *tileset)
{
	int	map_width_in_pixels;
	int	map_height_in_pixels;

	tileset->img_path = "./RF_Catacombs_v1.0/tilesfloor.xpm";
	tileset->img = mlx_xpm_file_to_image(window->mlx, \
		tileset->img_path, &tileset->width, &tileset->height);
	tileset->addr = mlx_get_data_addr(tileset->img, \
		&tileset->bits_per_pixel, &tileset->line_length, &tileset->endian);
...
int	launcher(t_solong *solong, char **av)
{
	...
	if (!display_map(solong, &solong->window, &solong->tileset))
